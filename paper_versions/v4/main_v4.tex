...
\end{align*}
where $s$ is the reported seed.  For backward compatibility with the predecessor benchmark, the NumPy seed is the first 16 hexadecimal digits of
\texttt{SHA256(`v3|family|n|m|Gamma|seed')} reduced modulo $2^{32}$.  This seed derivation is platform-independent; the released environment record and result files remain authoritative because numerical distribution routines can vary across library versions.
