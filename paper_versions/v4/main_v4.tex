...
The evaluator yields a minimax interval bound for adaptive threshold decomposition.  We prove validity, singleton exactness, and a new dominance result: the exact minimax envelope bound is no larger than the bounded-threshold group-clique relaxation.  A protocol-fixed computational study then separates algebraic correctness, kernel scaling, common-search-tree behavior, end-to-end certificate time, robustness, and application-derived coefficient realism.  The contribution is the simultaneous parametric bound, not a new uncertainty set, a faster algorithm for one knapsack relaxation, or universal superiority of an integer solver.
...
\end{align}
This is the exactly-one specialization of the bounded-threshold clique construction of \citet{BusingGersingKoster2023}.
...
&\sum_{k,i,j}c_{ij}(b_k)y_{kij}
 \le\sum_k\mu_k C(b_k).
\end{align*}
Strong duality applies because the primal epigraph is feasible and bounded.  Take any feasible dual solution and set
\begin{align*}
x_{ij}&:=\sum_k y_{kij},
...
All runs used one thread on an Apple M4 under macOS, Python 3.14.2, NumPy 2.4.4, and SciPy 1.17.1 with HiGHS.  Source data, raw timing repetitions, environment records, summaries, and generation scripts accompany the paper.

Four structured families isolate distinct difficulties: \emph{dense frontier} gives many competitive value--resource tradeoffs; \emph{correlated risk} couples value gains and deviations; \emph{near tie} produces unstable rankings; and \emph{many breakpoints} gives essentially unique deviations, so $B$ grows with $K$.  The primary panel uses six options per group, $\Gamma=\lfloor\sqrt n\rfloor$, $n\in\{360,720,1440\}$, five seeds per family--size cell, and five timing repetitions per method.  Method order alternates within an instance; the instance median is the sole statistical observation.  We report geometric-mean paired speedup, a 10,000-draw bootstrap CI that resamples seeds within each family--size cell, and an exact one-sided sign test.
...
Every family median exceeds one, from \PubFamilyBreakpoints$\times$ for many breakpoints to \PubFamilyCorrelated$\times$ for correlated risk, so the predeclared breadth gate passes.
...
\begin{table}[t]
\centering
\caption{Separate stress panel.  Each row contains one new instance per family; conclusions are scaling evidence, not a confidence claim.}
\label{tab:stress}
\small
...
\]
The percentile interval resamples seeds with replacement separately inside each family--size cell for 10,000 draws, preserving the factorial design.  The exact one-sided sign test uses $\Pr\{\operatorname{Binomial}(R,1/2)\ge W\}$, where $W$ is the number of instance-level wins.  No timing repetition enters either inferential calculation.
