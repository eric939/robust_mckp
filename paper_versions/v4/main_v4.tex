...
The paper makes four contributions.  First, it derives the cancellation and a two-envelope algorithm that evaluates a multiplier over all thresholds at once.  Second, it turns this evaluator into a valid minimax certificate and proves that the exact certificate is no weaker than the closest bounded-threshold group-clique relaxation.  Third, it embeds the certificate in an adaptive decomposition with explicit global-bound invariants and a separately scoped exact-integer extension.  Fourth, a protocol-fixed study distinguishes algebraic correctness, kernel scaling, common-search-tree behavior, end-to-end certificate time, robustness, and application-derived coefficient realism.  The contribution is the simultaneous parametric bound, not a new uncertainty set, a faster algorithm for one knapsack relaxation, or universal superiority of an integer solver.

\section{Robust MCKP and prior-art boundary}\label{sec:problem}

\subsection{The \texorpdfstring{$\Gamma$}{Gamma}-robust MCKP}

Let $i\in\{1,\ldots,n\}$ index groups and let $J_i$ be the finite option set of group $i$.  Option $(i,j)$ has value $v_{ij}\in\R$, nominal resource contribution $a_{ij}\in\R$, and nonnegative deviation $d_{ij}\ge0$.  Exactly one option is selected from every group.  With an integer uncertainty budget $\Gamma\in\{0,\ldots,n\}$, the problem is
\begin{align}
...
The finite evaluated bound $\widehat U_{\Lambda_I}(I)$ remains valid by \Cref{prop:validity}, but a deliberately coarse multiplier set need not inherit \Cref{thm:dominance}.  The implementation combines a fixed logarithmic grid with bounded scalar refinement; the common-trace experiment separately checks its realized relationship to $Q(I)$.
...
The implementation evaluates a fixed 26-point logarithmic grid of multipliers and then applies bounded scalar search around the best interior grid point.  Grid traces are cached once and reused by every child interval.  The theorem's storage bound is attained with ragged sorted group arrays.  The released vectorized code instead pads them to an $n\times m_{\max}$ rectangle, so its general storage is $O(B+n m_{\max}+K)$; this reduces to $O(B+K)$ in every reported equal-menu experiment.  It reports the smallest value actually evaluated, advanced by one binary64 representable number toward $+\infty$.  The propositions concern real arithmetic; the implementation replaces exact feasibility by the stated numerical tolerance and is therefore a solver-tolerance certificate, not an interval-arithmetic proof.  In exact arithmetic, imperfect multiplier minimization can weaken the bound but cannot invalidate it.
...
\qquad
\mathrm{UB}:=\max\left\{\mathrm{LB},U^{\rm disc},\max_{I\in\mathcal P}\widehat U_{\Lambda_I}(I)\right\},
\]
...
The revised protocol was serialized before the sparse-comparator confirmatory rerun; its SHA-256 digest is
\nolinkurl{20fd417c6765573b98c88b648abb8af74775e5d0f7d0796aa8a0097853f211ce}.
This is a prospectively locked internal protocol, not an externally registered study.  All runs used one thread on an Apple M4 under macOS, Python 3.14.2, NumPy 2.4.4, and SciPy 1.17.1 with HiGHS.  Source data, raw timing repetitions, environment records, summaries, and generation scripts accompany the paper.

Four structured families isolate distinct difficulties: \emph{dense frontier} gives many competitive value--resource tradeoffs; \emph{correlated risk} couples value gains and deviations; \emph{near tie} produces unstable rankings; and \emph{many breakpoints} gives essentially unique deviations, so $B$ grows with $K$.  The primary panel uses six options per group, $\Gamma=\lfloor\sqrt n\rfloor$, $n\in\{360,720,1440\}$, five seeds per family--size cell, and five timing repetitions per method.  Method order alternates within an instance; the instance median is the sole statistical observation.  We report geometric-mean paired speedup, a 10,000-draw design-stratified bootstrap interval that resamples seeds within each family--size cell, and an exact one-sided sign test.  These summaries quantify variation across the generated instances in the fixed factorial design; they do not define a superpopulation of all robust MCKPs.

The end-to-end comparator is the bounded-threshold LP of \citet{BusingGersingKoster2023}, specialized with one clique/GUB inequality per exactly-one group as in \eqref{eq:clique-main} and assembled in sparse CSR form.  Both methods use the same best-first splitting policy, fixed-threshold LP routine, endpoint--midpoint candidate rule, scaled tolerance $\varepsilon=10^{-6}$, and time limit including preprocessing.  They differ only in the interval upper bound: the comparator solves a bounded-threshold clique LP, whereas the compressed method evaluates the group-envelope Lagrangian bound.
...
Forty held-out irregular instances use unequal menu sizes, repeated and near-repeated deviations, signed objectives, and $\Gamma$ values from 0 to $n$.  We compare the compressed trace with (i) the dense baseline-cost trace, (ii) direct evaluation of \eqref{eq:cancelled-D}, (iii) independently optimized interval bounds, (iv) the independent fixed-MCKP LP routine at every threshold, and (v) a complete fixed-threshold LP scan.  The maximum absolute discrepancy is \PubValidationMaxError, and the minimum root-bound slack over the full scan is \PubValidationMinSlack.  Both protocol-fixed validation gates pass.
...
To isolate the interval oracles from adaptive-tree effects, a common-trace ablation evaluates both bounds on the same \PubTraceIntervals{} dyadic intervals from \PubTraceCases{} independently generated instances.  The compressed oracle wins \PubTraceWins{} instance timings with a \PubTraceGeoSpeedup$\times$ geometric-mean speedup; its bound is no larger than the clique bound on \PubTraceDominancePct{} of matched intervals.  This comparison includes preprocessing and sparse LP assembly.
...
\label{tab:kernel}
\small
\input{tables/v4_kernel_publication.tex}
\end{table}

\subsection{Primary result}

Both methods reach tolerance on all \PubPrimaryCases{} primary instances.  The compressed method wins all \PubPrimaryWins{} paired timings ($p=\PubPrimarySignP$), with geometric-mean speedup \PubPrimaryGeoSpeedup$\times$ and 95\% design-stratified bootstrap interval [\PubPrimaryCILow, \PubPrimaryCIHigh].  It evaluates a median \PubPrimaryMedianThetaPct{} of thresholds as fixed LPs.  Final lower bounds differ by at most \PubPrimaryMaxDifference{} relatively, and its root bound never exceeds the clique root bound within numerical tolerance.  All \PubRepeatWins{}/\PubRepeatBlocks{} repeat blocks are wins (minimum \PubRepeatMinSpeedup$\times$); median within-instance coefficients of variation are \PubCompressedMedianCV{} and \PubCliqueMedianCV{}.  Across an instance's interval solves, the clique comparator uses a median \PubCliqueMedianNnz{} sparse-matrix nonzeros and \PubCliqueMedianCSRMiB{} MiB of cumulative CSR arrays.  The right panel of \Cref{fig:scaling} displays instance- and size-level scaling.

\begin{table}[t]
\centering
\caption{Primary end-to-end certificate comparison.  Each reported time and evaluation count is the median across instances of that size after taking the within-instance median over five balanced timing repetitions.  Aggregate speedup treats each instance once.  C and Q denote the compressed and clique certificates.}
\label{tab:primary}
\small
\resizebox{\textwidth}{!}{\input{tables/v4_primary_publication.tex}}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/v4_speedup_scaling.pdf}
...
Every family median exceeds one, from \PubFamilyBreakpoints$\times$ for many breakpoints to \PubFamilyCorrelated$\times$ for correlated risk, so the protocol-fixed breadth gate passes.

\subsection{Robustness and stress evidence}

The 36-instance robustness panel changes one dimension at $n=720$: $\Gamma=1$, twelve-option menus, or $\Gamma=\lfloor0.2n\rfloor$.  Every instance reaches tolerance.  The compressed method wins 11 of 12 sparse-budget timings and all 24 timings in the other configurations; every configuration-level bootstrap interval remains above one.  The larger fraction of fixed thresholds under the large budget partly explains its lower speedup than the wide-menu panel.
...
\small
\resizebox{\textwidth}{!}{\input{tables/v4_robustness_publication.tex}}
\end{table}

The stress panel is descriptive rather than inferential: it uses four families, one new seed, and two repetitions at each of 2,880 and 5,760 groups.  All eight instances reach tolerance with identical final lower bounds.  The geometric-mean speedup is \PubStressGeoSpeedup$\times$ overall and \PubStressNHighSpeedup$\times$ at 5,760 groups.  The many-breakpoints instances contain 14,401 and 28,801 thresholds, respectively.

\begin{table}[t]
\centering
\caption{Separate stress panel.  Each row aggregates one new instance per family; reported times are medians across those four instances after taking the within-instance median over two balanced repetitions.  Conclusions are scaling evidence, not a confidence claim.}
\label{tab:stress}
\small
\input{tables/v4_stress_publication.tex}
\end{table}
\FloatBarrier

\subsection{Exact integer integration audit}

The integer integration in \Cref{cor:integer-integration} was audited separately from the confirmatory LP-certificate experiment.  Twelve instances use 30 groups, all four families, three new seeds, two balanced timing repetitions, and a five-second limit.  The envelope and clique interval searches use the same fixed-threshold branch-and-bound and certify 10 of 12 instances each; complete threshold enumeration certifies 11, and compact SCIP certifies all 12.  Whenever at least two methods certify, their objectives differ by at most $1.14\times10^{-13}$.  On the ten jointly certified envelope--clique cases, the geometric-mean clique/envelope time ratio is 1.00, showing that fixed-threshold integer search, rather than interval-bound computation, dominates at this scale.  The many-breakpoints family remains the decisive limitation: compact SCIP certifies all three instances while each interval method certifies one.  This audit validates the exact integration and its accounting invariant, but it does not support a claim of universal integer-solver superiority; the paper's computational claim remains acceleration of the complete LP-bound certificate.

\subsection{Application-derived external panel}

The pricing model in \Cref{sec:pricing-specialization} also supplies a post-confirmatory realism check.  We use the UCI Online Retail data set, which contains 541,909 transaction records \citep{Chen2015UCI}.  The released calibration reads the first 200,000 records; after positive-price/quantity filtering and the eight-observation SKU rule, 192,451 observations contribute to 2,549 SKU aggregates.  These aggregates calibrate the segment mix, price levels, volumes, and uncertainty scales of nine semi-synthetic pricing portfolios; documented elasticity and menu-generation assumptions remain modeled rather than observed.  The panel uses 360, 720, and 1,440 groups, twelve price choices, three new seeds per size, and three balanced repetitions.  Both methods reach tolerance, and the compressed method wins \PubApplicationWins{}/\PubApplicationCases{} instances with geometric-mean speedup \PubApplicationGeoSpeedup$\times$ (95\% cell-stratified bootstrap interval [\PubApplicationCILow, \PubApplicationCIHigh]), rising to \PubApplicationNHighSpeedup$\times$ at 1,440 groups.  This panel tests scale and coefficient realism; demand estimation is outside the algorithmic scope, and the nonrandom row cap and modeled demand curves preclude causal or transaction-level validation claims.

\subsection{Scope and limitations}

The experiments validate the envelope identity, retained LP certificate, and speed advantage over the tested sparse clique-LP implementation, not universal integer-solver dominance.  No standard benchmark targets this relaxation, so the confirmatory generators are controlled and the external panel remains semi-synthetic; the implementation uses Python/SciPy on one architecture, and the stress sample is small.  Streaming could reduce dense-oracle storage without changing its $\Theta(BK)$ arithmetic, while compiled code could shift constants.  The exact audit shows that fixed-threshold integer search can erase the interval-oracle advantage and that compact SCIP remains preferable on the many-breakpoints family.  DnC+, specialized fixed-MCKP solvers, and the oracle are therefore complementary.

\section{Conclusion}\label{sec:conclusion}

After sorting, exactly-one group envelopes evaluate one multiplier over all thresholds in $O(B+K\log(B+1))$ time and $O(B+K)$ working storage.  The exact minimax bound is valid, singleton-exact, and no larger than the bounded-threshold group-clique LP.  The protocol-fixed study finds algebraic agreement within tolerance, \PubPrimaryWins{} of \PubPrimaryCases{} primary wins, a \PubPrimaryGeoSpeedup$\times$ geometric-mean certificate speedup, robustness across three altered regimes, and increasing size-level separation through 5,760 groups.  The released exact integration preserves valid global gap accounting but also identifies the boundary of the contribution: integer subproblem work can dominate, and compact SCIP remains stronger on many-breakpoints instances.  Extensions to multiple robust constraints and broader partitioned models are the next theoretical step.
...
\end{align*}
where $s$ is the reported seed.  To reproduce the frozen release exactly, the NumPy seed is the first 16 hexadecimal digits of
\texttt{SHA256(`v3|family|n|m|Gamma|seed')} reduced modulo $2^{32}$.  The literal \texttt{v3} prefix is a legacy implementation namespace only; it does not denote a manuscript dependency or publication lineage.  This seed derivation is platform-independent; the released environment record and result files remain authoritative because numerical distribution routines can vary across library versions.
